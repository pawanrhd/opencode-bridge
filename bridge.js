...
let   loggedSampleEvent = false // one-time diagnostic dump, see runModelAttempt
...
function buildParts(messages, tools) {
  ...
  for (const m of messages) {
    ...
    if (m.role === "assistant" && m.tool_calls?.length) {
      ...
      // NOTE: OpenCode's /session/{id}/message and /prompt_async endpoints
      // only accept part types "text" | "file" | "agent" | "subtask" — a
      // literal type:"tool-call" part is rejected with a 400 BadRequest.
      // Fold prior tool calls into a plain text block instead.
      for (const tc of m.tool_calls) {
        const args = (() => { try { return JSON.parse(tc.function?.arguments ?? "{}") } catch { return {} } })()
        parts.push({ type: "text", text: `[TOOL CALL] ${tc.function?.name ?? tc.name}(${JSON.stringify(args)})` })
      }
      continue
    }

    // Tool result messages — same restriction as above, fold into text
    // rather than a type:"tool-result" part.
    if (m.role === "tool") {
      const resultText = typeof m.content === "string" ? m.content : JSON.stringify(m.content ?? "")
      parts.push({ type: "text", text: `[TOOL RESULT for ${m.tool_call_id}]\n${resultText}` })
      continue
    }
...
async function runModelAttempt({ sessionId, providerID, modelID, msgParts, probeTimeoutMs, onFirstToken, onDelta, onToolCall }) {
    ...
    while (!done && !probeExpired) {
      ...
      while ((idx = buffer.indexOf("\n\n")) !== -1) {
        ...
        if (evt.type === "message.part.updated") {
          if (!loggedSampleEvent) {
            loggedSampleEvent = true
            logger.info(`SAMPLE EVENT (first message.part.updated seen): ${JSON.stringify(evt).slice(0, 2000)}`)
          }
          // This OpenCode version's events carry the part's current text
          // directly (part.text), not a separate "delta" field, and don't
          // reliably indicate here whether the part belongs to the user's
          // own submitted message or the assistant's reply. Trusting this
          // for content risked echoing the user's own prompt back as if it
          // were the answer (confirmed happening). So these events are now
          // used only as a "the model is doing something" signal to cancel
          // the probe timeout — the real answer always comes from session
          // history below, filtered strictly to the assistant's message.
  ...
  // The real answer is always sourced here, from session history, never
  // from the live event stream (see note above). Filter strictly to
  // messages with role "assistant" — earlier code grabbed whatever was
  // simply last in the list, which sometimes was the user's own just-
  // submitted message, and echoed it back as if it were the reply.
  if (done) {
    try {
      const msgData = await ocGetList(`/session/${sessionId}/message`)
      const msgList = Array.isArray(msgData) ? msgData : (msgData.messages ?? msgData.data ?? [])
      const assistantMsgs = msgList.filter(m => (m.role ?? m.info?.role) === "assistant")
      const last  = assistantMsgs[assistantMsgs.length - 1]
      const parts = last?.parts ?? last?.info?.parts ?? []

      const text = parts.filter(p => p.type === "text" && p.text).map(p => p.text.trim()).filter(Boolean).join("\n\n")
      const reasoning = parts.filter(p => p.type === "reasoning" && p.text).map(p => p.text.trim()).filter(Boolean).join("\n\n")
      const tcParts = parts.filter(p => p.type === "tool-call")

      if (text) {
        textAcc = text
        if (!gotFirst) { gotFirst = true; onFirstToken?.() }
      ...
      for (const tc of tcParts) {
        const t = { toolCallId: tc.toolCallId, toolName: tc.toolName, toolArgs: tc.toolArgs }
        toolCalls.push(t)
        onToolCall?.(t)
      }

      if (!assistantMsgs.length) {
        logger.error(`no assistant message found in /session/${sessionId}/message for ${modelID} (${msgList.length} total messages) — send this count + a sample message shape to Claude`)
      } else if (!text && !reasoning) {
        logger.error(`assistant message for ${modelID} found but had no text/reasoning parts — response may genuinely be empty`)
      }
    } catch (err) {
      logger.error(`reconciliation fetch failed for ${modelID}: ${err.message}`)
    }
  }
